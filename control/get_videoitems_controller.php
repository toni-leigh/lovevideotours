<?php
    ...
    // dev_dump($category);
        ...
        else
        {
            ...
            {
                $scats=site_query("select * from Category where (parentID=".$category["categoryID"]." or categoryID=".$category["categoryID"].") and itemTypeID=3","get_items_controller.php - get scats",0);
                while ($scat=mysql_fetch_array($scats)) $sparams["cats"][]=$scat["categoryID"];
            }
            else
            {
                ...
                $sparams["cats"]=$_SESSION["sparams"]["cats"];
            }
        }
    ...
    $sparams["map"]=0;
    if ($category["categoryID"]==3)
        $items=get_video_items(array("cats"=>array(3)));   
    else
